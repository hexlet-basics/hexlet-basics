...
class LessonTester
  ...
  def run(lesson_version, language_version, code, user)
    ...
    command = "docker pull #{language_version.docker_image} --quiet && docker run --rm --net none #{volume} #{language_version.docker_image} timeout 4 make --silent -C #{lesson_version.path_to_code} test"
