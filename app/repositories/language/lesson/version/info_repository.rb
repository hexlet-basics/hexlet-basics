...
module Language::Lesson::Version::InfoRepository
  ...
  included do
    ...
    scope :not_finished_by, ->(user) do
      joins("LEFT OUTER JOIN language_lesson_members ON language_lesson_members.lesson_id = language_lesson_version_infos.language_lesson_id AND language_lesson_members.user_id = #{user.id}")
    end
  end
end
