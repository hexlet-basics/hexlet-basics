...
| 18 | Клиент amoCRM | kiota 1.34.1 (.NET-бинарь) + 3 рантайм-модуля kiota | ogen в режиме клиента | ~~**пересмотреть**~~ — сделано в `6124aac8` |
...
> Сделано в `6124aac8`: ogen справился со спецификацией (`hal+json` и
> `problem+json` декодируются штатно), kiota убран из `mise.toml` и `go.mod`.
> Классификация ошибок переехала в обёртку `ht.Client`; попутно исправлены
> повторы, дублировавшие лиды при нечитаемом 2xx.
