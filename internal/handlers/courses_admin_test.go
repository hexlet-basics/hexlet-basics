...
func TestAdminCreateCourse(t *testing.T) {
	...
	created, err := h.Client.AdminCreateCourse(ctx, &api.CourseInput{
		...
		Progress:                 api.NewNilCourseProgress(api.CourseProgressInDevelopment),
	...
	assert.Equal(t, api.CourseProgressInDevelopment, created.Progress.Value)
...
func TestAdminUpdateCourse(t *testing.T) {
	...
	updated, err := h.Client.AdminUpdateCourse(ctx, &api.CourseInput{
		...
		Progress:                 api.NewNilCourseProgress(api.CourseProgressCompleted),
	...
	require.NoError(t, err)
	assert.Equal(t, http.StatusOK, h.LastStatus())
	assert.Equal(t, api.CourseProgressCompleted, updated.Progress.Value)
