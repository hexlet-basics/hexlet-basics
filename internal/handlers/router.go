...
// patterns are strictly more specific, so Go's ServeMux routes them first and
// only unmatched requests fall through to ogen. The upload route is the
// generated server too, behind a body-size cap.
//
...
func NewRouter(
	...
	transport.Handle("POST /admin/attachments", http.MaxBytesHandler(generated, UploadBodyLimit))
