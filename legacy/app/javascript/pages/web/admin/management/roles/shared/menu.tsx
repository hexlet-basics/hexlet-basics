import { useTranslation } from "react-i18next";
import {
  CrudHorizontalMenu,
  type CrudHorizontalMenuItem,
} from "@/components/CrudHorizontalMenu";
import * as Routes from "@/routes.js";
import type { StaffRoleCrud } from "@/types";

type Props = {
  data?: StaffRoleCrud;
};

export function Menu({ data }: Props) {
  const { t } = useTranslation();

  const items: CrudHorizontalMenuItem[] = [
    {
      href: Routes.admin_management_roles_path(),
      label: t(($) => $.helpers.crud.list),
    },
    {
      href: Routes.new_admin_management_role_path(),
      label: t(($) => $.helpers.crud.add),
    },
  ];

  if (data) {
    items.push({
      href: Routes.edit_admin_management_role_path(data.id),
      label: t(($) => $.helpers.crud.editing),
    });
    items.push({
      href: Routes.admin_management_role_permission_path(data.id),
      label: t(($) => $.admin.management.role_permissions.show.link),
    });
  }

  return <CrudHorizontalMenu items={items} />;
}
